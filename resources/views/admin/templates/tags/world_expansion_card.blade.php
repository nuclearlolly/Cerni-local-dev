                            ...
                            <div class="row col-12 py-1 logs-table-row attachment-row">
                                ...
                                <div class="col-6 col-md-6 attachment-row-select">
                                    @if ($attachment['attachment_type'] == 'Figure')
                                        {!! Form::select('data[attachment_id][]', $figures, $attachment['attachment_id'], ['class' => 'form-control figure-select', 'placeholder' => 'Select Figure']) !!}
                                    @elseif($attachment['attachment_type'] == 'Fauna')
                                        {!! Form::select('data[attachment_id][]', $faunas, $attachment['attachment_id'], ['class' => 'form-control fauna-select', 'placeholder' => 'Select Fauna']) !!}
                                    @elseif($attachment['attachment_type'] == 'Flora')
                                        {!! Form::select('data[attachment_id][]', $floras, $attachment['attachment_id'], ['class' => 'form-control flora-select', 'placeholder' => 'Select Flora']) !!}
                                    @elseif($attachment['attachment_type'] == 'Faction')
                                        {!! Form::select('data[attachment_id][]', $factions, $attachment['attachment_id'], ['class' => 'form-control faction-select', 'placeholder' => 'Select Faction']) !!}
                                    @elseif($attachment['attachment_type'] == 'Concept')
                                        {!! Form::select('data[attachment_id][]', $concepts, $attachment['attachment_id'], ['class' => 'form-control concept-select', 'placeholder' => 'Select Concept']) !!}
                                    @elseif($attachment['attachment_type'] == 'Location')
                                        {!! Form::select('data[attachment_id][]', $locations, $attachment['attachment_id'], ['class' => 'form-control location-select', 'placeholder' => 'Select Location']) !!}
                                    @elseif($attachment['attachment_type'] == 'Event')
                                        {!! Form::select('data[attachment_id][]', $events, $attachment['attachment_id'], ['class' => 'form-control event-select', 'placeholder' => 'Select Event']) !!}
                                    @endif
                                </div>
