...
@if ($character->profile->parsed_text)
    <div class="card mb-4" style="border-top-right-radius: 1.8em; border-top-left-radius: 1.7em; box-shadow: 0px 0px 6px 3px rgba(70, 0, 136, 0.08);">
        <button class="collapsible text-center">
            <h5><i class="fas fa-star fa-shake-icon"></i> Profile ▾</h5>
        </button>
                    ...
                    @if (Auth::check() && Auth::user()->hasPower('manage_characters'))
                        <div class="tab-pane fade" id="settings-{{ $character->slug }}">
                            ...
                            <div class="text-right">
                                ...
                                @endif <a href="#" class="btn btn-outline-info btn-sm reupload-image" data-id="{{ $image->id }}">Reupload Image</a> <a href="#"
                                    class="btn btn-outline-danger btn-sm delete-image" data-id="{{ $image->id }}">Delete</a>
                            </div>
                        </div>
                </div>
                @endif
